...
FLASH_Status FLASH_EnableWriteProtection(u32 page)
{
    ...
    u8           i;
    for (i = 0; i < 4; i++) {
        ret = FLASH_ProgramOptionHalfWord((FLASH_OB_BASE + 8 + i * 2), ~(page >> (i * 8)));
        if (ret != FLASH_COMPLETE) {
            break;
        }
    }
...
FLASH_Status FLASH_UserOptionByteConfig(OB_IWDG_TypeDef OB_IWDG, OB_STOP_TypeDef OB_STOP, OB_STDBY_TypeDef OB_STDBY)
{
    FLASH_OPTB_Enable();
    FLASH->CR |= FLASH_CR_OPTPG;
    FLASH_OB->USER = OB_IWDG;
    FLASH_OB->USER |= OB_STOP;
    FLASH_OB->USER |= OB_STDBY;
    FLASH_OB->USER |= 0xF8;
